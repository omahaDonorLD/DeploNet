	...
	for i in individual:
		for j in individual:
			if (i.id != j.id) and (j.id > i.id):
				...
				if distance <= uavrange:
					connections = connections + 1 # update connections
					i.neighbordrones.append(j)
					j.neighbordrones.append(i)

	labels=list()
	labels.append(individual[0])
	for k in labels :
		##print(len(labels))
		for z in k.neighbordrones:
			if z not in labels :
				labels.append(z)

	##print("Here ", len(labels), " and ", len(individual))
	'''
	for k in individual :
		check=False
		for j in individual :
			if k.id in j.neighbordrones :
				#print(k.id, " in neighborhood of ", j.id, " : ", j.neighbordrones)
				check=True
		if not check :# print 
			print(k.id)
	print("THE END")
...
def evaluate_weighted(individual):
	...
	if check_graph_connectivity(G) == True:
		...
		quality = (q1*1000) + (q2*100) + q3 # CO + FTO + RO, where RO is sum of |SCi| and CO is the union on grounds covered
	else:
		quality = -1
	return quality,
...
def initPredefPop(pcls, ind_init, input_data):
	##print("checking value of input_data : ", input_data)
	return pcls(ind_init(input_data[c]) for c in range(1,len(input_data)))

def init_geometric(num_drones):
	## random but still ensures that connectivity of drones of is kept
	r = uavrange
	list_drones =  list()
